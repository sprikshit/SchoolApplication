...
<script>
    ...
    function fetchData() {
        fetch('https://schoolapi-3yo0.onrender.com/')
...
function viewDetails(loginID) {
    ...
    fetch(`https://schoolapi-3yo0.onrender.com//${loginID}`)
        .then(response => response.json())
        .then(data => {
            // Redirect to the new page (singleteacher.php) and pass the data as a query parameter
            const queryParams = new URLSearchParams({ teacherData: JSON.stringify(data) });
